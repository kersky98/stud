...
struct AddressHasher {
  ...
  size_t operator()(const Address& value)const{
    size_t r1 = shash(value.city);
    size_t r2 = shash(value.street);
    size_t r3 = ihash(value.building);
    size_t x = 2'946'901;
...
struct PersonHasher {
  // реализуйте структуру
  size_t operator()(const Person& value)const{
    size_t r1 = shash(value.name);
    size_t r2 = ihash(value.height);
    size_t r3 = dhash(value.weight);
    size_t r4 = ahash(value.address);
    size_t x = 2'946'901;
