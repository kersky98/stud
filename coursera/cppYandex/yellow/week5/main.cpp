...
public:
    ...
    void Learn() const {cout << "Student: " << GetName() << " learns" << endl;}
...
public:
    ...
    void Teach() const {cout << "Teacher: " << GetName() << " teaches: " << _subject << endl;}
    void Walk(const string& destination) const override {cout << "Teacher: " << GetName() << " walks to: " << destination << endl;}
...
void VisitPlaces(Person& person, vector<string> places) {
    for (const auto& p : places) {person.Walk(p);}}
