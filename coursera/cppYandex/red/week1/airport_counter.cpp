...
/*  struct s{
  typename vector<Item>::iterator first, last;
  typename vector<Item>::iterator begin() const{
    return first;
  }
  typename vector<Item>::iterator end() const{
    return last;
  }
  size_t size() const{
    return distance(first, last);
  }
  typename vector<Item>::iterator begin(){
    return first;
  }
  ...
  Items GetItems() const{
    ...
    for(size_t i = 0; i < a.size(); i++ ){
      //items.push_back(make_pair(static_cast<TAirport>(i), a[i]));
      //items.emplace_back(static_cast<TAirport>(i), a[i]);
      items[i] = make_pair(static_cast<TAirport>(i), a[i]);
    }
    //items_s.first = begin(items);
    //items_s.last = end(items);
    //return {begin(items), end(items)};
    return items;
  }
